...
def find_max_cpu_load(jobs):
    min_heap, max_cpu_load, curr_cpu_load = list(), 0, 0
    for job in sorted(jobs, key=lambda x: x.start):
        while min_heap and min_heap[0].end <= job.start:
            curr_cpu_load -= heappop(min_heap).cpu_load
